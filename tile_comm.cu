...
struct TileDesc {
    ...
    int   group_tiles;      // number of tiles represented by send_elems
...
/* Shared grouped all-reduce helpers.  A group is laid out as
 * [tile first_tile][tile first_tile+1]... in contiguous device memory.
 */
...
static AllreduceGroup make_allreduce_group(
    int first_tile, int group_tiles, float* send_ptr, float* recv_ptr,
    size_t output_tile_elems
) {
    const size_t elems = static_cast<size_t>(group_tiles) * output_tile_elems;
    if (elems > static_cast<size_t>(INT_MAX)) {
        fprintf(stderr, "ERROR: grouped allreduce element count exceeds INT_MAX.\n");
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    AllreduceGroup g;
    g.first_tile = first_tile;
    g.group_tiles = group_tiles;
    g.send_elems = static_cast<int>(elems);
    g.send_ptr = send_ptr;
    g.recv_ptr = recv_ptr;
    return g;
}

static void allreduce_group_blocking(const AllreduceGroup& g, MPI_Comm comm) {
    MPI_CHECK(MPI_Allreduce(
        g.send_ptr, g.recv_ptr, g.send_elems, MPI_FLOAT, MPI_SUM, comm
    ));
}

static void iallreduce_group(const AllreduceGroup& g, MPI_Comm comm, MPI_Request* req) {
    MPI_CHECK(MPI_Iallreduce(
        g.send_ptr, g.recv_ptr, g.send_elems, MPI_FLOAT, MPI_SUM, comm, req
    ));
}
...
void comm_thread_fn(
    ...
    auto try_launch = [&]() {
        while ((int)active.size() < max_inflight) {
            ...
            AllreduceGroup group;
            group.first_tile = desc.tile_id;
            group.group_tiles = desc.group_tiles;
            group.send_elems = desc.send_elems;
            group.send_ptr = desc.send_ptr;
            group.recv_ptr = desc.recv_ptr;
            iallreduce_group(group, comm, &inf.req);
...
int main(int argc, char** argv) {
    ...
    if ((mode == Mode::OVERLAP || mode == Mode::TILED) && num_tiles % aggregate_tiles != 0) {
        if (rank == 0)
    ...
    if ((mode == Mode::OVERLAP || mode == Mode::TILED) &&
    ...
    // Serialized modes: one buffer per tile.
    // Tiled mode: one grouped buffer per deferred communication group.
    // Overlap mode: num_buffers with reuse.
    const int num_groups = (mode == Mode::TILED || mode == Mode::OVERLAP)
        ? num_tiles / aggregate_tiles : num_tiles;
    const int n_bufs = (mode == Mode::OVERLAP) ? num_buffers
                         : (mode == Mode::TILED ? num_groups : num_tiles);
    ...
    for (int b = 0; b < n_bufs; ++b) {
        size_t send_recv_bytes = (mode == Mode::OVERLAP || mode == Mode::TILED)
    ...
    auto run_tiled = [&](double t0_wall) {
        const int tiled_groups = num_tiles / aggregate_tiles;

        for (int grp = 0; grp < tiled_groups; ++grp) {
            ...
            for (int t = 0; t < aggregate_tiles; ++t) {
                ...
                float* proj_dest = send_buffers[grp] +
                                   static_cast<size_t>(t) * output_tile_elems;
                project_one_at_offset(tile, tile, proj_dest);
            }
        }
        CUDA_CHECK(cudaStreamSynchronize(compute_stream));
        CUDA_CHECK(cudaStreamSynchronize(proj_stream));
        ...
        for (int grp = 0; grp < tiled_groups; ++grp) {
            int first_tile = grp * aggregate_tiles;
            AllreduceGroup g = make_allreduce_group(
                first_tile, aggregate_tiles, send_buffers[grp], recv_buffers[grp],
                output_tile_elems
            );

            double group_comm_start = MPI_Wtime();
            allreduce_group_blocking(g, MPI_COMM_WORLD);
            double group_comm_end = MPI_Wtime();

            for (int t = 0; t < aggregate_tiles; ++t) {
                int tile = first_tile + t;
                comm_start_wall[tile] = group_comm_start;
                comm_end_wall[tile] = group_comm_end;
            }

            float* out = output_device +
                         static_cast<size_t>(first_tile) * output_tile_elems;
            CUDA_CHECK(cudaMemcpyAsync(
                out, recv_buffers[grp],
                static_cast<size_t>(g.send_elems) * sizeof(float),
                cudaMemcpyDeviceToDevice, comm_cuda_stream
            ));
        }
        CUDA_CHECK(cudaStreamSynchronize(comm_cuda_stream));
    ...
    auto run_overlap = [&](double t0_wall) {
        ...
        for (int grp = 0; grp < num_groups; ++grp) {
            ...
            desc.group_tiles  = aggregate_tiles;
    ...
    } else if (mode == Mode::TILED) {
        int tiled_groups = num_tiles / aggregate_tiles;
        for (int g = 0; g < tiled_groups; ++g) {
            int first = g * aggregate_tiles;
            local_comm_ms += (comm_end_wall[first] - comm_start_wall[first]) * 1e3;
        }
    } else {
        for (int t = 0; t < num_tiles; ++t)
    ...
    if (rank == 0) {
        ...
        } else if (mode == Mode::OVERLAP || mode == Mode::TILED) {
